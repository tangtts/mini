...
Page({
    ...
    getDetailById(id) {
        ...
        db.collection('records').doc(id).get().then(async res => {
            ...
            let data = res.data;
            let distance = (await this.calcDistance({ longitude: data.loc.longitude, latitude: data.loc.latitude })).toFixed(2);
            this.setData({
                distance,
    ...
    async setAddress(data) {
        let distance = (await this.calcDistance({ longitude: data.longitude, latitude: data.latitude })).toFixed(2);
        this.setData({
    ...
    async calcDistance({ latitude, longitude }) {
